...
# NYS Census
```{r}
nys <- read.csv("../Data/NYS.csv", nrows = 61)
colnames(nys) <- c("Fact",	"Fact Note",	"NewYork",	"Value Note for New York")
nys <- nys %>%
  select(Fact, NewYork)
kable(nys)
```
...
dat2$score <- numeracy$score
dat2$health <- ifelse(dat_orignal$JAq4<=4, 1, 0)
```
...
varlist <- c("numeracy", "score", "educ", "income", "race", "hisp")
...
Multivariate model with binary numeracy status as predictor. 

```{r, warning=FALSE}
full1 <- glm(health ~ educ + hisp + race + income + numeracy, 
...
Final model: health ~ educ + hisp + numeracy. AUC = 81.23%.
...
rocplot(dat2$health, predict.glm(final1, dat2, type = "response"), title = "ROC plot")
title(main = "ROC plot")
```

Multivariate model with continuous numeracy score as predictor. 
```{r, warning=FALSE}

full2 <- glm(health ~ educ + hisp + race + income + score, 
            data = dat2, family = "binomial")
...
Final model: health ~ educ + hisp + score. AUC = 81.26%.
```{r, message=FALSE}
# ROC curve

rocplot(dat2$health, predict.glm(final2, dat2, type = "response"), title = "ROC plot")
title(main = "ROC plot")
```
