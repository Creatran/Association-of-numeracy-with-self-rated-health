...
# Wrap up of the previous regression results (without weights consideration)
...
## Adding survey weights to ESP data  
...
## About the plausible value in PIAAC
reference: 
...
Plausible Values (PVs) allows PIAAC dataset information to be saved at the case level to estimate proficiency at the national or subgroup level o Each case's PVs reflect not only that individual's performance on the small number of items s/he answered, but also the performance of similar respondents on the rest of the PIAAC assessment    
...
## Correlation between health status and numeracy score in PIAAC
