...
# path <- "../Data"
# path <- "/Users/zihanyang/Documents/Capstone_ESP/Data"
...
# Regression rsults (without weights consideration)
## Association between numeracy and self-reported health status   
...
## Association between numeracy and visits to doctors   
...
```{r}
ESP %>% ggplot() +
  geom_boxplot(aes(x=health_status_1, y=numeracy, fill=health_status_1)) +
  labs(title="Box plot of numeracy score and health status (Not Poor VS. Poor) ",y="Numeracy Score", x = NULL, fill="health status") +
  theme_bw() +
  # theme(plot.title = element_text(hjust = 0.5,size = 18)) +
  scale_fill_brewer(palette="Dark2")

ESP %>% ggplot() +
  geom_boxplot(aes(x=health_status_2, y=numeracy, fill=health_status_2)) +
  labs(title="Box plot of numeracy score and health status (Good VS. Not Good) ",y="Numeracy Score", x = NULL, fill="health status") +
  theme_bw() +
  # theme(plot.title = element_text(hjust = 0.5,size = 18)) +
  scale_fill_brewer(palette="Dark2")
...
comp1 <- rbind(c("Gender", "", ""), comp1[1:2,],
              ...
              c("Race", "", ""), comp1[14:18,],
              c("Ethnicity", "", ""), comp1[19:20,]) %>% as.data.frame()

# append p-values of fisher's test
comp1$pval <- sapply(list(1:2,3:6,7:9,10:13,14:18,19:20), 
                     function(x){
                       mat <- fisher.mat[,x]
...
kable(comp1, row.names = F) %>% 
  ...
  row_spec(c(1,4,9,13,18,24), bold = T)

```

We will try to use other methods like post-hoc analysis for multiple pairwise adjustment for anova in exact fisher's test.
...
## ESP (with weights)
Based on the summary table above, `education`, `income` and `race` are three variables that have different distributions in ESP data and NYS dataset. However, among these three variables only `education` is a significant covariate in previous regression results. It appears the distributions of `income` and `race` are not associated with people's health status. 

So, here we only add survey weights to adjust the educationl level distribution in ESP data.    

***Question:*** <p style="text-decoration: underline;">Do we also need to add survey weights on income and race even though they are not significant covariates?</p>
...
3. For observation purporse, we might consider add income and race, the weight value would change. <br>
4. The weight would change if we will combine the design bias. <br>
Therefore, the one given above is the optimized solution we could provide with available information now.


***Question:*** <p style="text-decoration: underline;">What is the final supposed goal for here `PIACC vs. ESP` and `ESP vs. NYS`: Inference /Observation/Prediction?</p>

***Question:*** <p style="text-decoration: underline;">So far we only consider the post-strafication weights without considering the design weights considering the sampling ratio.</p>
...
**2. Cnsensus on usage of weight:** <br>
...
Compute weights separately but sequentially.***(Question)***
...
ESP.weighted <- surv.wt(ESP.weighted, wt.edu, 7, "educ")
...
comp2 <- rbind(c("Education", "", ""), comp2[1:3,],
               c("Income", "", ""), comp2[4:8,],
               c("Hisp", "", ""), comp2[9:11, ]) %>% as.data.frame()
rownames(comp2) <- NULL
...
comp2$pval <- c(p.educ, rep("",3), p.income, rep("",5), p.hisp, rep("", 3))

kable(comp2, row.names = F) %>%
  kable_styling(full_width = F) %>%
  row_spec(c(1,5, 11), bold = T)
```
...
***Question:*** <p style="text-decoration: underline;">Should we use either PV1 or the combination from PV1 to PV10?</p>
...
```{r}
sum(is.na(raw_PIAAC$PVNUM1) != is.na(raw_PIAAC$PVNUM2))
```
...
1. Do we also need to add survey weights on income and race even though they are not significant covariates?

2.What is the final supposed goal for here `PIACC vs. ESP` and `ESP vs. NYS`: Inference /Observation/Prediction?

3. So far we only consider the post-strafication weights without considering the design weights considering the sampling ratio. (EG: Table 3 in [ESP methodology](https://sri.cornell.edu/sri/polls/files/esp/2019/ESP19%20-%20Introduction%20and%20Methodology%20Report.pdf))

4. Post-Stratification weights: realted to the demographical freatures of population 
Compute weights separately but sequentially.

5. Should we use either PV1 or the combination from PV1 to PV10?
